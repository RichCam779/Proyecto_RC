...
const gethistorialById = async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT * FROM historial WHERE id_historial = $1', [id]);
...
const createhistorial = async (req, res) => {
  try {
    ...
    const query = `INSERT INTO historial (${keys.join(', ')}) VALUES (${placeholders}) RETURNING *`;
...
const updatehistorial = async (req, res) => {
  try {
    ...
    const query = `UPDATE historial SET ${setClause} WHERE id_historial = $${keys.length + 1} RETURNING *`;
...
const deletehistorial = async (req, res) => {
  try {
    const { id } = req.params;
    let query = `UPDATE historial SET estado = 'Inactivo' WHERE id_historial = $1 RETURNING *`;
    
    try {
      const { rows } = await pool.query(query, [id]);
      if (rows.length === 0) {
        query = `DELETE FROM historial WHERE id_historial = $1 RETURNING *`;
