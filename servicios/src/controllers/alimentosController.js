...
// Obtener por ID
const getalimentosById = async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT * FROM alimentos WHERE id_alimento = $1', [id]);
...
// Crear
const createalimentos = async (req, res) => {
  try {
    const keys = Object.keys(req.body);
    const values = Object.values(req.body);
    const placeholders = keys.map((_, i) => '$' + (i + 1)).join(', ');
    
    const query = `INSERT INTO alimentos (${keys.join(', ')}) VALUES (${placeholders}) RETURNING *`;
...
// Actualizar
const updatealimentos = async (req, res) => {
  try {
    const { id } = req.params;
    const keys = Object.keys(req.body);
    const values = Object.values(req.body);
    ...
    const query = `UPDATE alimentos SET ${setClause} WHERE id_alimento = $${keys.length + 1} RETURNING *`;
...
const deletealimentos = async (req, res) => {
  try {
    const { id } = req.params;
    let query = `UPDATE alimentos SET estado = 'Inactivo' WHERE id_alimento = $1 RETURNING *`;
    
    try {
      const { rows } = await pool.query(query, [id]);
      if (rows.length === 0) {
        query = `DELETE FROM alimentos WHERE id_alimento = $1 RETURNING *`;
